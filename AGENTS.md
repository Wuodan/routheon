...
- `traefik/routheon-server/`: Lightweight Python service aggregating `/v1/models` and `/stats`.
...
- Linting/formatting: run language-specific tools only on touched files—`ruff check traefik/routheon-server --fix`, `shellcheck`/`bash -n` for scripts, `yamllint traefik` for YAML, and pick the equivalent best-practice linter/formatter for any other file type you modify—so best-practice rules stay enforced without rewriting the entire repo.
